        ...
        <div className="hidden md:flex gap-6 items-center font-bold text-white">
          {trails.map((trail) => (
            <a
              ...
              // Item ativo: azul-escuro (a MESMA cor de hover do menu) e, ao
              // passar o mouse, volta ao branco (a cor do item não selecionado).
              // Não selecionado: branco (herdado) → hover azul-escuro.
              className={`transition-colors cursor-pointer ${
                activeCategory === trail.id
                  ? 'text-brand-dark hover:text-white'
          ...
          <span className="flex items-center gap-4 pl-4 border-l border-white/20">
            ...
            {hasCauses && (
              <Link to="/causas" className="hover:text-brand-dark transition-colors flex items-center gap-1">
        ...
        <div className="md:hidden bg-brand-blue border-t border-brand-blue px-6 py-4 flex flex-col gap-4 font-bold text-white shadow-xl absolute w-full">
          {trails.map((trail) => (
            <a
              ...
              href={trail.href}
              onClick={(e) => handleClick(e, trail.id)}
              className="hover:text-brand-dark transition-colors block py-2 cursor-pointer"
            >
              {trail.label}
            </a>
          ))}
          ...
          <span className="border-t border-brand-cream/20 pt-3 flex flex-col gap-3">
            {hasCauses && (
              <Link to="/causas" onClick={() => setIsOpen(false)} className="hover:text-brand-dark transition-colors flex items-center gap-2">
                <HeartHandshake className="w-4 h-4" />
                Causas para apoiar
              </Link>
            )}
